...
@section('container')
    @if (auth()->user()->username == 'itsupport' || auth()->user()->username == 'sudosu')
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a class="btn btn-sm btn-primary"
                                            href="{{ url('/dashboard/it/' . $item->id) }}"><i
                                                class="bi bi-info-square"></i></a>
                                        <a class="btn btn-sm btn-warning"
                                            href="{{ url('/dashboard/it/' . $item->id . '/edit') }}"
                                            @if ($item->approve == '1') style="pointer-events:none; opacity:0.5;" @endif><i
                                                class="bi bi-pencil-square"></i></a>
                                        <form action="{{ url('/dashboard/it/' . $item->id) }}" method="post"
                                        ...
                                        @if (auth()->user()->name == 'IT Support')
                                            <form action="{{ route('it.approve', $item->id) }}" method="post"
                                                class="d-inline">
                                                @csrf
                                                @method('put')
                                                ...
                                                <button
                                                    class="btn btn-sm {{ $item->approve ? 'btn-success' : 'btn-secondary' }}"
