...
<div class="container-fluid pt-4 px-4">
    <div class="bg-light text-center rounded p-4">
        ...
        <div class="d-flex align-items-center justify-content-between mb-4">
            <h6 class="mb-0">Daftar Surat Divisi Finance AP</h6>
            <a href="{{ url('/dashboard/finap/create') }}" class="btn btn-primary"><i class="bi bi-file-text-fill"></i> Tambah Surat</a>
        </div>
            ...
            <table class="table text-start align-middle table-bordered table-hover mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <a class="btn btn-sm btn-primary" href="{{ url('/dashboard/finap/' . $item->id) }}"><i class="bi bi-info-square"></i></a>
                            <a class="btn btn-sm btn-warning" href="{{ url('/dashboard/finap/' . $item->id . '/edit') }}" @if($item->approve == "1") style="pointer-events:none; opacity:0.5;" @endif><i class="bi bi-pencil-square"></i></a>
                            <form action="{{ url('/dashboard/finap/' . $item->id) }}" method="post" class="d-inline delete-form">
                                @method('delete')
                                @csrf
                                <button type="button" class="btn btn-sm btn-danger border-0 delete-button"><i class="bi bi-trash"></i></button>
                            </form>
                            <a class="btn btn-sm btn-info" href="{{ url('/dashboard/finap/' . $item->id . '/cetak') }}" target="_blank"><i class="bi bi-printer"></i></a>
                            @if (auth()->user()->name == "LALA")

                            <form action="{{ route('finap.approve', $item->id) }}" method="post" class="d-inline">
                                @csrf
                                @method('put')
